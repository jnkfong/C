...
//infinite loop
    while(1)
    {
        ...
        //which will initialize the delay
        getInput();

        //begin LED sequence on LSb
        LATB=0x01;
        delayMs(delayPot);
        ...
        for (i=0; i<11; i++)
        {
            getInput(); //refresh pot value for delay
            LATB=LATB << 1;
            delayMs(delayPot);
        }
...
void getInput(void)
{
    ...
    delayPot = ADC1BUF0; // yes, then save ADC value to variable
}
/*******************************************************************************
