 ...
 * reaches this line, it must pick up the pail, turn around and follow the path
 * back with the pail to reach an identical horizontal line, where the pail is
 * dropped (beyond the line). To complete this challenge, the robot must follow
...
unsigned int holdPail=0; //check if pail is held
...
void initialize(void)
{
  ...
  OC1R = 675;           // Write the duty cycle for the 1st PWM pulse at 0 degrees 900 600
...
void decide(void)
{
        ...
        if (leftOpto < LEFT_THRESHOLD && rightOpto < RIGHT_THRESHOLD)
        {
            ...
            else
            {
              ...
              // Turn on Right LED (RB8)
              LATBbits.LATB8 = 1;
              //If robot is not holding a pail, lift pail
              if (holdPail == 0)
              {
                  brakes(); //stop robot                
                  liftArm(); //lift pail
                  ...
                  holdPail++; //intialize robot is holding pail
              }
              //If robot is holding a pail, drop pail
              else if (holdPail == 1)
              {
                  brakes(); //stop robot                 
                  lowerArm(); //lower pail
                  ...
                  delayMs(800);
                  turnB(); //turn robot with reverse
                  holdPail--; //intialize robot has dropped pail
               }
             }
        }
...
//Turning with reverse while contain a pail (prevent continue hooking onto pail)
void turnB (void)
{
    while (rightOpto > RIGHT_THRESHOLD && leftOpto > LEFT_THRESHOLD)
    {
...
void liftArm(void)
{   
    OC1R = 900 ; //lift arm up to ## degrees after hooked onto pail 1125 875
}
//Lower robot arm down
void lowerArm(void)
{   
    OC1R=675; //lower arm to 0 degrees to release pail 900 600
}
// end main.c